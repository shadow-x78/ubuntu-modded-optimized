...
### 🐛 Fixed
- **APT NO_PUBKEY Error:** Fixed `NO_PUBKEY 871920D1991BC93C` by downloading the official binary `ubuntu-archive-keyring.gpg` file directly from `archive.ubuntu.com` into the container's `trusted.gpg.d` directory. This completely circumvents any `.deb` extraction or `dpkg` dependencies during initialization, elegantly bypassing the `dpkg-deb: Function not implemented` PRoot errors.
