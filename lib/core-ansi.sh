...
umo_banner_full() {
    ...
    # exactly 22 chars wide (uniform, leading spaces preserved) so the mark
    ...
    printf "%b%*s%s%b\n" "$UMO_GRAD_2" "$_pad" '' "$_l9" "$UMO_NC"
    printf "%b%*s%s%b\n" "$UMO_GRAD_1" "$_pad" '' "$_l10" "$UMO_NC"
    printf "%b%*s%s%b\n" "$UMO_GRAD_1" "$_pad" '' "$_l11" "$UMO_NC"
    printf '\n'

    _tag="Ubuntu Modded Optimized · v${UMO_VERSION:-4.17.3}"
