...
umo_de_xfce4() {
    ...
    gnome-icon-theme 2>&1 | _apt_filter || true'
    if [ "${UMO_XFCE4_WHISKERMENU:-0}" = "1" ]; then
        _pkgs="$_pkgs
timeout 600 apt-get install -y --no-install-recommends xfce4-whiskermenu-plugin 2>&1 | _apt_filter || true"
    fi
    _umo_de_build "$_pkgs"
