...
_run_installer() {
    ...
    _script="${UMO_INSTALL_DIR:?}/root/install-apps.sh"
    {
        ...
        printf '%s\n' '    fi'
        printf '%s\n' '    _umo_wire_default_browser'
        printf '%s\n' '}'
        printf '%s\n' '_umo_wire_default_browser() {'
        printf '%s\n' '    _wb=/usr/local/bin/umo-browser'
        printf '%s\n' '    command -v falkon >/dev/null 2>&1 || return 0'
        printf '%s\n' '    [ -x "$_wb" ] || return 0'
        printf '%s\n' '    _step "Wiring Falkon As The Default Browser..."'
        printf '%s\n' '    mkdir -p /usr/share/xfce4/helpers 2>/dev/null || true'
        printf '%s\n' '    cat > /usr/share/xfce4/helpers/umo-falkon.desktop << UMOFALKON'
        printf '%s\n' '[Desktop Entry]'
        printf '%s\n' 'Version=1.0'
        printf '%s\n' 'Icon=falkon'
        printf '%s\n' 'Type=X-XFCE-Helper'
        printf '%s\n' 'Name=Falkon Web Browser (UMO)'
        printf '%s\n' 'StartupNotify=false'
        printf '%s\n' 'NoDisplay=true'
        printf '%s\n' 'X-XFCE-Binaries=/usr/local/bin/umo-browser;'
        printf '%s\n' 'X-XFCE-Category=WebBrowser'
        printf '%s\n' 'X-XFCE-Commands=%B;'
        printf '%s\n' 'X-XFCE-CommandsWithParameter=%B "%s";'
        printf '%s\n' 'UMOFALKON'
        printf '%s\n' '    for _u in /root /home/umo; do'
        printf '%s\n' '        [ -d "$_u" ] || continue'
        printf '%s\n' '        mkdir -p "$_u/.config/xfce4" 2>/dev/null || true'
        printf '%s\n' '        _hr="$_u/.config/xfce4/helpers.rc"'
        printf '%s\n' '        [ -f "$_hr" ] || : > "$_hr" 2>/dev/null || true'
        printf '%s\n' '        if grep -q "^WebBrowser=" "$_hr" 2>/dev/null; then'
        printf '%s\n' '            sed -i "s|^WebBrowser=.*|WebBrowser=umo-falkon|" "$_hr" 2>/dev/null || true'
        printf '%s\n' '        else'
        printf '%s\n' '            printf "WebBrowser=umo-falkon\\n" >> "$_hr" 2>/dev/null || true'
        printf '%s\n' '        fi'
        printf '%s\n' '        _ml="$_u/.config/mimeapps.list"'
        printf '%s\n' '        [ -f "$_ml" ] || printf "[Default Applications]\\n" > "$_ml" 2>/dev/null || true'
        printf '%s\n' '        for _mt in x-scheme-handler/http x-scheme-handler/https text/html; do'
        printf '%s\n' '            awk -v k="$_mt" -v v="org.kde.falkon.desktop" '"'"''
        printf '%s\n' '                BEGIN { insec = 0; have = 0; written = 0 }'
        printf '%s\n' '                /^\[Default Applications\][ \t]*$/ { have = 1; insec = 1; print; next }'
        printf '%s\n' '                /^\[/ {'
        printf '%s\n' '                    if (insec && !written) { print k "=" v; written = 1 }'
        printf '%s\n' '                    insec = 0; print; next'
        printf '%s\n' '                }'
        printf '%s\n' '                insec && index($0, k "=") == 1 { print k "=" v; written = 1; next }'
        printf '%s\n' '                { print }'
        printf '%s\n' '                END {'
        printf '%s\n' '                    if (insec && !written) print k "=" v'
        printf '%s\n' '                    else if (!have) { print ""; print "[Default Applications]"; print k "=" v }'
        printf '%s\n' '                }'
        printf '%s\n' '            '"'"' "$_ml" > "${_ml}.umo" 2>/dev/null && mv -f "${_ml}.umo" "$_ml" 2>/dev/null || rm -f "${_ml}.umo" 2>/dev/null || true'
        printf '%s\n' '        done'
        printf '%s\n' '        if [ "$_u" = "/home/umo" ]; then'
        printf '%s\n' '            chown 1000:1000 "$_hr" "$_ml" 2>/dev/null || true'
        printf '%s\n' '        fi'
        printf '%s\n' '    done'
        printf '%s\n' '    update-alternatives --install /usr/bin/x-www-browser x-www-browser "$_wb" 200 >> "$_LOG" 2>&1 || true'
        printf '%s\n' '    _ok "Falkon Wired As Default Browser (XFCE Helper + MIME + x-www-browser)"'
        printf '%s\n' '}'
        printf '%s\n' '_umo_setup_code_repo() {'
        ...
        printf '%s\n' '        fi'
        printf '%s\n' '        _udb="$_uh/.config/umo-vscode-data/User/globalStorage/state.vscdb"'
        printf '%s\n' '        if [ -f "$_udb" ] && command -v sqlite3 >/dev/null 2>&1; then'
        printf '%s\n' '            sqlite3 "$_udb" "INSERT OR REPLACE INTO ItemTable (key, value) VALUES ('"'"'sync.enable.globalState'"'"', 0); INSERT OR REPLACE INTO ItemTable (key, value) VALUES ('"'"'sync.enable.profiles'"'"', 0);" 2>/dev/null || true'
        printf '%s\n' '        fi'
        printf '%s\n' '    done'
        printf '%s\n' '}'
        printf '%s\n' 'timeout 600 apt-get update >> "$_LOG" 2>&1 || true'
